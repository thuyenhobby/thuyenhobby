...
export function ResourceCard({ resource, variant = "inventory" }: ResourceCardProps) {
  ...
  return (
    <article
      className={cn(
        "group relative overflow-hidden rounded-2xl border border-amber-900/15 bg-amber-50/65 p-3 shadow-sm transition duration-200 dark:border-amber-300/15 dark:bg-amber-950/20 md:p-4",
    ...
    >
      ...
      <div className="flex items-start gap-2.5 pl-2 md:gap-3">
        <span
          className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-lg border border-amber-700/15 bg-white/70 font-mono text-xs font-bold text-amber-800 dark:bg-amber-950 dark:text-amber-200 md:size-9"
          aria-hidden="true"
        >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h3 className="mt-1 line-clamp-2 text-sm font-semibold leading-5 text-stone-950 dark:text-stone-50 md:leading-6">
                {resource.title}
              </h3>
            </div>
            {resource.favorite ? (
          ...
          <p className="mt-1.5 line-clamp-1 text-xs leading-5 text-stone-600 dark:text-stone-300 md:line-clamp-2">
            {resource.description}
          </p>
          <div className="mt-2.5 flex flex-wrap items-center gap-1.5 md:mt-3">
          ...
          {resource.url ? (
            <a
              ...
              aria-label={`Open memory ${resource.title}`}
              className="focus-ring mt-3 inline-flex rounded-md text-xs font-bold text-amber-800 underline-offset-4 hover:underline dark:text-amber-200"
            >
              open memory ↗
            </a>
          ) : null}
