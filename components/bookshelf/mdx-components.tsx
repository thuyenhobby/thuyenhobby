...
const components: ComponentProps<typeof ReactMarkdown>["components"] = {
  h2: ({ children }) => <h2 className="mt-7 text-2xl font-semibold tracking-tight md:mt-10">{children}</h2>,
  h3: ({ children }) => <h3 className="mt-6 text-xl font-semibold tracking-tight md:mt-8">{children}</h3>,
  p: ({ children }) => <p className="mt-4 leading-7 text-muted md:mt-5 md:leading-8">{children}</p>,
  a: ({ href, children }) => {
  ...
  ul: ({ children }) => <ul className="mt-4 list-disc space-y-2 pl-6 text-muted md:mt-5">{children}</ul>,
  ol: ({ children }) => <ol className="mt-4 list-decimal space-y-2 pl-6 text-muted md:mt-5">{children}</ol>,
  li: ({ children }) => <li className="leading-7">{children}</li>,
  blockquote: ({ children }) => <blockquote className="mt-5 border-l-4 border-accent/40 bg-foreground/[0.03] px-4 py-3 text-muted md:mt-6 md:px-5">{children}</blockquote>,
  ...
  img: ({ src, alt }) => {
    ...
    return (
      <Image
        ...
        width={1200}
        height={675}
        className="mt-5 h-auto w-full rounded-2xl border border-border object-cover md:mt-6"
      />
    );
  },
};
